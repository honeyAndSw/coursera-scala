...
trait GameDef {
   ...
   *   0 o o o o
   *   1 o o o o
   *   2 o # o o    # is at position Pos(2, 1) (x = row = 2, y = col = 1)
   *   3 o o o o
   *
   *   ^
   *   |
  ...
  case class Pos(x: Int, y: Int) {
    ...
    def equals(that: Pos): Boolean = this.x == that.x && this.y == that.y
  }
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def neighbors: List[(Block, Move)] =
      List((left, Left), (right, Right), (up, Up), (down, Down)).filter(_._1.isLegal)
    ...
    def legalNeighbors: List[(Block, Move)] = for {
      direct <- neighbors
      indirect <- direct._1.legalNeighbors
    } yield indirect
